...
//   image - pointer to image to rasterize
//   tx,ty - image offset (applied after scaling)
//   scalex - horizontal image scale
//   scaley - vertical image scale
//   dst - pointer to destination image data, 4 bytes per pixel (RGBA)
//   w - width of the image to render
//   h - height of the image to render
//   stride - number of bytes per scaleline in the destination buffer
void nsvgRasterizeStretched(NSVGrasterizer* r,
...
static void nsvg__flattenShapeStroke(NSVGrasterizer* r, NSVGshape* shape, float scalex, float scaley)
{
	...
	float lineWidth = shape->strokeWidth * minScale;
	...
	for (path = shape->paths; path != NULL; path = path->next) {
		...
		if (shape->strokeDashCount > 0) {
			...
			dashLen = (shape->strokeDashArray[idash] - dashOffset) * minScale;
			...
			for (j = 1; j < r->npoints2; ) {
				...
				if ((totalDist + dist) > dashLen) {
					...
					dashState = !dashState;
					idash = (idash+1) % shape->strokeDashCount;
					dashLen = shape->strokeDashArray[idash] * minScale;
					// Restart
					cur.x = x;
					cur.y = y;
...
void nsvgRasterize(NSVGrasterizer* r,
				   ...
				   unsigned char* dst, int w, int h, int stride)
{
  nsvgRasterizeStretched(r, image,  tx,  ty,  scale,  scale, dst,  w,  h,  stride);
		   ...
		   unsigned char* dst, int w, int h, int stride)
{
	...
	for (shape = image->shapes; shape != NULL; shape = shape->next) {
		...
		if (shape->stroke.type != NSVG_PAINT_NONE && (shape->strokeWidth * minScale) > 0.01f) {
			nsvg__resetPool(r);
			r->freelist = NULL;
			r->nedges = 0;
